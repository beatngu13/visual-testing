...
var skxwEv = {
    init: function () {
        ...
        var $_skxwLi2 = $("#skxw_a .bd li");
        var skxwLiLen2 = $_skxwLi2.length;
        var str2 = "";
        for (i = 0; i < skxwLiLen2; i++) {
            str2 += '<li></li>';
        };
        $("#skxw_a .hd").append(str2);
        $("#skxw_a").slide({
            titCell: ".hd li",
            mainCell: ".bd",
            effect: "leftLoop",
...
var playVideo = {
    ...
    playVideos: function () {
        $(".focusBox .videoPlayBtn").click(function () {
			...
			var curIndex = 0;
			var nextHref = '';
			var videoList = $('.focusBoxWrap .videoPlayBtn a');
			var videoListHref = [];
			var videoIframe = $("#player_container iframe");
			var setVideo = true;
			
			for(var i=0; i<videoList.length; i++){
				videoListHref.push(videoList[i].href)
			};
			
			
			for(var j=0;j<videoListHref.length;j++){
				if(curHref==videoListHref[j]){
						curIndex = j;
				}
			};
			
			//console.log("点击选择当前视频："+curIndex);
			...
			window.addEventListener('message', function (event) {
				if(setVideo){
					...
					curIndex++;
					//console.log("videoListHref.length"+videoListHref.length)
					if(event.data === "videoEnd"){
						if(curIndex == videoListHref.length){
							curIndex = 0;
						}
						//console.log("curIndex视频iframe准备播放的下一个"+curIndex)
						//setIframeSrc(videoListHref[curIndex])
						$("#player_container iframe").attr('src',videoListHref[curIndex]);
					}
					setTimeout(isSetVideo,1000);
					function isSetVideo(){
						setVideo = true;
					}
				}
			...
			//视频iframe赋值
			var spSrc = $(this).find("a").attr("href");
			var spDom = '<iframe width="655" height="440" frameborder="0" scrolling="no" src="'+spSrc+'"></iframe>'
			$("#player_container").html(spDom)
        });
...
var newsing = {
    ...
    videoPlay: function () {
        $("#xwjxs .newsVideo .play").click(function () {
            ...
            var ifrStr = isLtIe10?('<iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="' + _src + '"></iframe>'):('<video loop="loop" controls autoplay src='+ _srcMp4 +'></video>');
